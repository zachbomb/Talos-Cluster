...
- R4. **Severity taxonomy with objective rubric.** Three severities — `critical`, `warning`, `info` — with a written decision rubric so future PRs land alerts consistently:
   ...
   - **Flap classification rule:** alerts that transition between firing and resolved 4+ times within a 30-minute window MUST be reclassified down one severity tier (critical → warning, warning → info) for the remainder of that window. **Implementation:** the canonical PromQL is `sum_over_time(changes(ALERTS{alertname="<name>",alertstate="firing"}[1m])[30m:1m]) >= 4` (subquery counting 1-minute change-edges across the 30-minute window). The naive `changes(ALERTS_FOR_STATE{...}[30m]) >= 4` does NOT work because `ALERTS_FOR_STATE` disappears from the result set on resolve rather than transitioning — `changes()` over a vanishing series under-counts cycles. Equivalent and simpler: a recording rule emitting a 1/0 firing indicator and counting edges over the window. Either form is acceptable; both are unit-tested via `promtool test rules` against synthetic series fixtures (R6). Prevents "alarm fatigue from flapping" anti-pattern where a single intermittent condition produces continuous critical pages.
...
- R14. **HITL state machine.** Each alert instance has one of these states: `Pending` (buttons posted, no click yet) → `Approved` (playbook running) → `Succeeded` / `Failed` / `Timeout` (terminal); `Pending` → `Snoozed` (silenced for fixed duration) → `Pending` (after snooze expires); `Pending` → `Ignored` (terminal-for-this-alert-instance, but Prometheus continues evaluating — Ignored prevents the *current* button cycle from prompting again, not the alert from re-firing on its next evaluation); `Pending` → `Auto-resolved` (alert resolved before any click — buttons disabled, follow-up message posted, subsequent clicks return "stale"). Each transition emits a structured Loki event (R17). **Keying:** state is keyed primarily on Prometheus alert `fingerprint` (the canonical immutable identifier across re-evaluations); `alertname` is denormalized for query convenience but never used as a uniqueness key. Concurrent alerts have independent state. Approve is **idempotent** per alert instance: the first click runs the playbook; subsequent clicks (network blip, double-click) acknowledge "already executed" without re-running.
   ...
   - **Reconciliation-on-startup contract.** On pod restart, the receiver MUST run a reconciliation pass before accepting new interactions. The reconciler (a) lists all CRs in non-terminal states (`Pending`, `Snoozed`, `Approved`); (b) for each `Snoozed` CR, queries AM for the matching alert's current state — if resolved, calls `expireSilence` and transitions Snoozed → Auto-resolved (closes the dead-pod-during-resolution-webhook race in R15); (c) for each `Approved` CR, lists Kubernetes Jobs labelled `alerting.local/fingerprint=<fp>` — adopts the still-running Job rather than launching a new one, watches it to completion, and transitions state on exit code; if the Job is missing entirely (cleaned up while pod was dead), transitions to `Timeout` and emits `remediation_failed` per R16. This contract is identical for both R14 paths (CRD or Robusta). The reconciler MUST run to completion before the receiver's HTTP listener accepts Discord interactions, preventing a window where an Approve click during reconciliation creates a duplicate Job.
...
- R18. **Alerting-pipeline meta-monitoring with documented sink independence.** Three safeguards, with explicit acknowledgement of remaining shared infrastructure layers:
  - (a) **Watchdog → uptime-kuma push (notification-provider independent).** PrometheusRule `Watchdog` fires every 1 minute with `severity: info` and the corresponding Alertmanager route emits a heartbeat HTTP POST to an uptime-kuma push monitor. uptime-kuma alerts on heartbeat-stop after 3 consecutive missed heartbeats (≈3-4 minutes) via its OWN notification channels (Pushover or ntfy.sh — explicitly NOT Discord/Notifiarr/Alertmanager), so "Alertmanager is down" or "Discord/Notifiarr API is failing" still surface. **Cadence trade-off:** 1-minute heartbeats with a 3-miss threshold give a ≈5-minute detection SLO with margin for one jittered POST; the previous 5-minute cadence would have made 10-minute detection a floor rather than a margin.
  ...
  - **Sink independence is at the notification-provider layer only — accepted residual risk.** The Watchdog → uptime-kuma → Pushover/ntfy path breaks the *Discord/Notifiarr/Alertmanager* circular dependency, but DOES still share lower layers with the alerting pipeline: (1) the Talos VM (uptime-kuma runs in-cluster); (2) Cilium CNI + BGP/L2 announcements; (3) Blocky/k8sgateway DNS for resolving notification provider hostnames; (4) UDM-SE WAN for egress. A failure at any of these layers silences both paths simultaneously. This is documented as accepted residual risk for the home-lab single-cluster topology. Mitigation candidates (off-cluster uptime-kuma on Proxmox LXC, secondary push-API path via a different DNS resolver, dual-WAN) are deferred to follow-up work; the current configuration is "best available within single-cluster constraint."
...
- R22. **BGP-recovery playbook.** When triggered (Approve from a `remediation: bgp-recovery` alert), the playbook launches a Kubernetes Job that SSHs to UDM-SE per R21 and runs `/data/on_boot.d/15-cluster-bgp.sh`. The playbook is idempotent (the underlying script is idempotent).
   - **Job shape:** the Job is labelled `alerting.local/fingerprint=<alert-fingerprint>` and `alerting.local/playbook=bgp-recovery` so the receiver's reconciler (R14) can find and adopt it on restart. `activeDeadlineSeconds: 240` (4 minutes — strictly less than the 5-minute rate-limit cooldown below, so a Timeout transition completes before the next eligible run). `backoffLimit: 0` (single attempt; retries are operator-driven, not automatic, per R16).
   - **SSH timeouts (mandatory):** the SSH client invocation MUST set `ConnectTimeout=10` (TCP setup), `ServerAliveInterval=15` + `ServerAliveCountMax=4` (60-second dead-connection detection), and `BatchMode=yes` (no interactive prompts). Without explicit timeouts, a hung dropbear or stuck network namespace transition would block the SSH session past the cooldown window and break the Failed/Timeout state transition.
   - **Replay-defense is layered:**
   - **Per-instance nonce:** the HITL receiver generates a single-use nonce on transition `Pending → Approved`, defined as `HMAC-SHA256(server-secret, alert.fingerprint || nonce_created_at)` truncated to 16 bytes hex. **Timestamp source is explicit:** `nonce_created_at` is the receiver's monotonic wall-clock unix-millisecond at the moment of nonce creation — NEVER the Discord interaction timestamp, NEVER the Prometheus alert `StartsAt`, NEVER any client-supplied value. Using a client-supplied timestamp would defeat the nonce because a replayed payload carries the original timestamp and produces the original nonce. The nonce is stored in the durable state record (R14) along with the `nonce_created_at` and a `key_id` field identifying which `hitl-replay-secret` version generated it. Playbook execution consumes the nonce (state transitions Approved → Succeeded/Failed/Timeout); subsequent attempts to execute the same `(fingerprint, nonce)` tuple are rejected with audit event `replay_rejected`. The HMAC key (`hitl-replay-secret`) is a SOPS Secret; rotation strategy and the `key_id` envelope behavior are deferred to /ce-plan.
...
R1, R2, R3, R4, R6, R18 (a/b/c — Watchdog, NotifiarrDown, PrometheusDown), R19, R20 (subset: webhook URLs + Pushover/ntfy creds for R18a sink). **Acceptance criteria (all CI-runnable except where noted):**
   - (a) Alertmanager pod running with PVC-backed silences surviving pod restart (integration test: create silence via API → delete pod → verify silence still active).
   - (b) `amtool` + `kubectl --dry-run=server` CI guard runs against an enumerated negative-fixture corpus committed to the repo: at minimum (i) `severity: warn` mistag (amtool severity-enum check fails), (ii) a route with `continue: true` reaching `discord-critical` (route-exclusivity check fails), (iii) AlertmanagerConfig with malformed `route.matchers` (kubectl --dry-run=server fails). All three fixtures MUST fail CI; the positive-control fixture (well-formed rule) MUST pass. A single fixture is insufficient — three regression classes need three fixtures.
   - (c) Watchdog plumbing check: observe ≥3 consecutive heartbeats arrive at uptime-kuma at the configured 1-minute cadence (≈3 minutes wall-clock); deliberately stop the Watchdog → verify uptime-kuma fires Pushover/ntfy notification within 5 minutes via the R18a independent path. **Note:** the 24-hour continuous-heartbeat property is an SLO line in Validation, NOT a Phase 1 CI gate (cannot run in CI; conflates "plumbing works" with "plumbing keeps working").
   - (d) Deliberately kill the Alertmanager pod (not just stop Watchdog) → verify uptime-kuma fires Pushover/ntfy notification within 5 minutes — proves the meta-monitor catches the system-it-monitors-down case, distinct from criterion (c)'s Watchdog-rule-down case.
   - (e) `promtool test rules` passes for the R4 flap-classification rule using synthetic series fixtures (4 firing edges in 10 min → demoted; 3 firing edges in 10 min → not demoted).
   
Outcome: Alertmanager is up, both receivers wired, severity routing live, meta-monitoring catches its own failures, but no remediation buttons.
...
R7, R8, R9, R10, R11, R12, R13, R14 (including durability + reconciliation tests), R15 (including all three silence-termination paths), R16, R17, R20 (remaining secrets + ConfigMap), R21, R22 (including nonce, SSH timeouts, Job adoption + rate-limit). **Acceptance criteria:**
   - All gameday scenarios in the Validation section pass on first run.
   - HITL receiver pod kill-and-recover preserves Pending state (R14 durability).
   - **Reconciliation-on-startup:** kill receiver while a CR is in `Snoozed` state with the underlying alert still firing AND while a separate CR is in `Approved` with a Job mid-flight. On restart, verify (i) the Snoozed CR's silence is reconciled against current alert state, (ii) the Approved CR's Job is adopted and watched to completion (NOT relaunched).
   - **Silence termination — three paths tested:** (i) cancel-on-resolve (alert resolves during snooze → silence expired automatically); (ii) natural AM expiry (let snooze run to natural expiry with alert still firing → fresh buttons posted with new correlation ID); (iii) manual unsnooze if exposed (operator clicks Unsnooze → silence expired). Path (ii) is the actual anti-pattern R15 exists to prevent and MUST be tested.
   - Unauthorized click produces `rejected_unauthorized` audit event.
   - Replay of an already-consumed nonce produces `replay_rejected` audit event.
   - SSH playbook hang test: simulate UDM-SE TCP-reachable-but-hung (e.g., iptables drop on dropbear's response packets) → verify Job hits `activeDeadlineSeconds` and transitions to Timeout within 4 minutes; verify a re-trigger after the 5-minute cooldown is permitted.
   
Outcome: BGP alert posts buttons; Approve runs the recovery playbook; everything is auditable; framework is complete.
...
- *Flapping?* → 4x in 30min downgrades severity one tier (R4); canonical PromQL pinned (`sum_over_time(changes(ALERTS{alertstate="firing"}[1m])[30m:1m]) >= 4`).
- *Nonce timestamp source?* → Server-generated wall-clock at nonce creation, never client-supplied (R22).
- *Reconciliation-on-startup?* → Required pre-listener pass that adopts in-flight Jobs and reconciles Snoozed silences (R14).
- *Playbook Job timeouts?* → SSH `ConnectTimeout=10` + `ServerAlive*`; Job `activeDeadlineSeconds: 240`; `backoffLimit: 0` (R22).
- *Watchdog SLO arithmetic?* → 1-min cadence + 3-miss threshold = ≈5-min detection floor (R18a).
- *Sink convergence layers?* → Documented as accepted residual risk (k8s API, Cilium, DNS, WAN); off-cluster uptime-kuma deferred (R18).
...
11. **R15 cancel-on-resolve.** Snooze a firing alert (1h silence active). Force-resolve the underlying condition during the silence window. **Expected:** AM emits resolve webhook; HITL receiver issues `expireSilence`; audit event `silence_cancelled_on_resolve` recorded; subsequent re-fire of the same condition produces fresh buttons (NOT swallowed by the silence).
12. **R15 natural-expiry-then-refire (the anti-pattern).** Snooze a firing alert. Let alert resolve naturally during the snooze. Let the silence run to natural AM expiry. Re-fire the underlying condition. **Expected:** buttons posted on re-fire (silence had been cancelled-on-resolve at step 1, so the natural-expiry path doesn't matter — but if cancel-on-resolve was bypassed, this step catches it).
13. **R14 reconciliation-on-startup.** With one CR in Snoozed (alert still firing) and another in Approved (Job mid-flight), kill the receiver pod. **Expected:** on restart, reconciler runs before HTTP listener accepts traffic; Snoozed CR's silence is reconciled against AM; Approved CR's Job is adopted (NOT a new Job launched); both reach correct terminal states.
14. **R22 SSH-hang test.** Simulate UDM-SE network-reachable-but-application-hung (e.g., iptables drop on dropbear's response packets). Approve a BGP-recovery alert. **Expected:** Job hits `activeDeadlineSeconds: 240`; state transitions to Timeout; `remediation_failed` follow-up alert fires; a re-trigger after the 5-minute cooldown is permitted (not blocked by stale Approved state).
...
- Production-grade re-review findings (revision 4 driver): R4 PromQL correctness fix, R22 nonce timestamp source, R14 reconciliation-on-startup contract, R22 Job shape + SSH timeouts, R18 convergence-layer documentation, R18a SLO arithmetic, Phase 1 negative-fixture corpus, R15 three-termination-path tests, gameday steps 11-14 (cancel-on-resolve, natural-expiry-then-refire, reconciliation, SSH-hang)
